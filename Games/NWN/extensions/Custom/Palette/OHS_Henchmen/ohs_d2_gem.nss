int StartingConditional()
{
  ...
  while (GetIsObjectValid(oThing) && !(GetBaseItemType(oThing)==BASE_ITEM_GEM && GetIdentified(oThing) && !GetItemCursedFlag(oThing)))
  {
    oThing = GetNextItemInInventory(oPC);
  }
  if (GetIsObjectValid(oThing))
  {
    SetLocalObject(oPC,"OHS_GIFT_GEM",oThing);
    SetCustomToken(783,GetName(oThing));
    return TRUE;
  }
  else
  {
