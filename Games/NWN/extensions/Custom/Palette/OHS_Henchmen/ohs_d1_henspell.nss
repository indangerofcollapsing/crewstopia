// This is the Bioware x0_d1_henchspell fixed by OldMansBeard to work
// for Paladins & Rangers. The change is to set bCheat to TRUE in
// the three ActionCastSpellAtObject() statements.
//
// Modified OMB 2006-01-30: bCheat is now picked up from a local variable.
// To be set in dialog for Paladin & Ranger spells, cleared otherwise.
// If bCheat is TRUE, caster level will be 10 regardless of actual level
// and this is not the wanted behaviour except when unavoidable.

// Modified again OMB 2006-08-20 for V1.5.3
// Bioware Patch 1.68 fixes the bug so the bCheat workaround
// can be turned off permanently for all classes.
...
void main()
{
    ...
    // int bCheat = GetLocalInt(OBJECT_SELF, "OHS_USE_SPELL_CHEAT");
    int bCheat = FALSE;  // V1.5.3
    ...
    {
        ClearAllActions();
        ActionCastSpellAtObject(nSpell, oTarget,METAMAGIC_ANY,bCheat);  // OMB
...
{
    // int bCheat = GetLocalInt(OBJECT_SELF, "OHS_USE_SPELL_CHEAT");  // OMB
    int bCheat = FALSE;  // V1.5.3
