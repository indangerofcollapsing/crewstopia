...
const int nMAX_PHENOTYPE_ALLOWED = 9; // @DUG 2;
...
void zep_Fly(object oFlyer, int nAttachWingType=nNO_WING_CHANGE, float fWalk_Speed=fDEFAULT_SPEED, string sItem_ResRef="")
{
   debugVarObject("zep_Fly()", OBJECT_SELF);
   debugVarObject("oFlyer", oFlyer);
    if (GetIsObjectValid(oFlyer)==FALSE) {debugMsg("flyer invalid"); return;}
    if (GetObjectType(oFlyer)!=OBJECT_TYPE_CREATURE){debugMsg("flyer not creature"); return;}

    int nAppearance = GetAppearanceType(oFlyer);
   debugVarInt("nAppearance", nAppearance);
    if (nAppearance == APPEARANCE_TYPE_INVALID){debugMsg("appearance invalid"); return;}
    ...
    debugVarInt("nPhenotype_C", nPhenotype_C);
    debugVarInt("nMAX_PHENOTYPE_ALLOWED", nMAX_PHENOTYPE_ALLOWED);
    if (nPhenotype_C >nMAX_PHENOTYPE_ALLOWED){ debugMsg("horse rider"); return;} //prevents horseriders from flying.
...
switch (nAppearance)
 {
    ...
    case APPEARANCE_TYPE_HUMAN:
      debugMsg("Dwarf/Elf/Gnome/HalfElf/HalfOrc/Halfling/Human");
      debugVarInt("nPhenotype_C", nPhenotype_C);
       if (nPhenotype_C == 0)
       {
           nNewPhenotype = n1st_Get_2DARow("phenotype", nCEP_PH_FLY,
              sCEP_PH_FLY);
       }
       else
       {
           nNewPhenotype = n1st_Get_2DARow("phenotype",nCEP_PH_FLY_L,
              sCEP_PH_FLY_L);
       }
       debugVarInt("nNewPhenotype", nNewPhenotype);

       if (nNewPhenotype != -1)
       {
          debugVarObject("oFlyer changing pheno", oFlyer);
          debugVarInt("new pheno", nNewPhenotype);
          SetPhenoType(nNewPhenotype,oFlyer);
          debugVarInt("oFlyer's new pheno", GetPhenoType(oFlyer));
          nBool=TRUE;
       }
       break;
 ...
 if ( (sItem_ResRef !="") && (n1st_HasItemInInventory(oFlyer,"",sItem_ResRef)==FALSE) )
  ...
  debugVarInt("current pheno", GetPhenoType(oFlyer));
}
