...
int TotalAndRemoveProperty(object oItem, int iType, int iSubType = -1)
{
   //debugVarObject("TotalAndRemoveProperty()", OBJECT_SELF);
   //debugVarObject("oItem", oItem);
   //debugVarInt("iType", iType);
   //debugVarInt("iSubType", iSubType);
...
itemproperty GetSpecificProperty(object oItem, int iType, int iSubType = -1, int iCostVal = -1, int iParam1 = -1, int iDuration = DURATION_TYPE_PERMANENT)
{
    itemproperty ip = GetFirstItemProperty(oItem);
    while(GetIsItemPropertyValid(ip)){
        if(GetItemPropertyType(ip)            == iType                          &&
...
void SetCompositeBonus(object oItem, string sBonus, int iVal, int iType, int iSubType = -1)
{
    //debugVarObject("SetCompositeBonus()", OBJECT_SELF);
    //debugVarObject("oItem", oItem);
    //debugVarString("sBonus", sBonus);
    //debugVarInt("iVal", iVal);
    //debugVarInt("iType", iType);
    //debugVarInt("iSubType", iSubType);
    ...
    switch(iType)
    {
        case ITEM_PROPERTY_ABILITY_BONUS:
            ...
            //debugVarInt("iCurVal", iCurVal);
            object oPossessor = GetItemPossessor(oItem);
            //debugVarObject("oPossessor", oPossessor);
            //debugVarInt("Leto value", GetPersistantLocalInt(oPossessor, "LetoAbility_"+IntToString(iSubType)));
            // @DUG iCurVal -= GetPersistantLocalInt(GetItemPossessor(oItem), "LetoAbility_"+IntToString(iSubType));
            //debugVarInt("adjusted iCurVal", iCurVal);
            ...
            //debugVarInt("iCurVal", iCurVal);
            //debugVarInt("iChange", iChange);
            if(iCurVal+iChange > 0)
            {
               itemproperty ip = ItemPropertyAbilityBonus(iSubType, iCurVal +
                  iChange);
            ...
            {
               itemproperty ip = ItemPropertyDecreaseAbility(iSubType, -1 *
                  (iCurVal + iChange));
               //debugVarItemProperty("ip", ip);
               AddItemProperty(DURATION_TYPE_PERMANENT, ip, oItem);
            }
...
void SetCompositeBonusT(object oItem, string sBonus, int iVal, int iType, int iSubType = -1)
{
    //debugVarObject("SetCompositeBonusT()", OBJECT_SELF);
    //debugVarObject("oItem", oItem);
    //debugVarString("sBonus", sBonus);
    //debugVarInt("iVal", iVal);
    //debugVarInt("iType", iType);
...
int GetItemPropertyDamageType(object oWeapon)
{
    ...
    switch(iDamageType)
    {
        ...
        case 5: return IP_CONST_DAMAGETYPE_PIERCING;    break; // @DUG piercing/bludgeoning
