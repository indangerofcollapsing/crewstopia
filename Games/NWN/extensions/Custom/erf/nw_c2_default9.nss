// *****************************************************************
//                        ***** NOTE *****
// This is included in the PrC Hakpacks, so if you want to customize
// behavior in your module, you must remove it from all Hakpacks
// (PrC, Override, ???) and import/merge into the module scripts.
// Why the hell Bioware made Hakpacks take precedence over local
// scripts, I Do Not Know.  What a fucking hassle.
// *****************************************************************
...
void main()
{
   ...
   */
   ...
   if (!GetPlotFlag(OBJECT_SELF))
   {
      if (testODBC())
      {
         // Check whether this creature is in the database.
         SQLExecDirect(
            "SELECT resref, name " +
              "FROM nwn_creatures " +
             "WHERE resref = '" + GetResRef(OBJECT_SELF) + "' " +
               "AND ( module_name = 'Bioware' " +
                "OR   module_name = 'CEP' " +
         ...
         {
            logError("NOTE: no nwn_creatures record found for " +
               objectToString(OBJECT_SELF));
            SQLExecDirect(
               "INSERT INTO nwn_creatures " +
                    "( resref, tag, name, cr, faction_id, race_id, " +
                    "  appearance_id, gender_id, is_pc, is_plot, " +
                    "  is_immortal, is_lootable, good_evil, law_chaos, " +
                    "  class1_id, class1_level, class2_id, class2_level, " +
                    "  class3_id, class3_level, palette_id, module_name " +
                    ") " +
               "VALUES " +
                    ...
                    ", " + IntToString(GetIsEnemy(GetFirstPC(), OBJECT_SELF) ?
                       STANDARD_FACTION_HOSTILE : STANDARD_FACTION_COMMONER) +
                    ", " + IntToString(GetRacialType(OBJECT_SELF)) + " " +
                    ", " + IntToString(GetAppearanceType(OBJECT_SELF)) + " " +
                    ", " + IntToString(GetGender(OBJECT_SELF)) +
                    ", " + (GetIsPC(OBJECT_SELF) ? "TRUE" : "FALSE") +
                    ", " + (GetPlotFlag(OBJECT_SELF) ? "TRUE" : "FALSE") +
                    ", " + (GetImmortal(OBJECT_SELF) ? "TRUE" : "FALSE") +
                    ", " + (GetLootable(OBJECT_SELF) ? "TRUE" : "FALSE") +
                    ", " + IntToString(GetAlignmentGoodEvil(OBJECT_SELF)) +
                    ", " + IntToString(GetAlignmentLawChaos(OBJECT_SELF)) +
                    ", " + IntToString(GetClassByPosition(1)) +
                    ", " + IntToString(GetLevelByPosition(1)) +
                    ", " + IntToString(GetClassByPosition(2)) +
                    ", " + IntToString(GetLevelByPosition(2)) +
                    ", " + IntToString(GetClassByPosition(3)) +
                    ", " + IntToString(GetLevelByPosition(3)) +
                    ", -1 " +
                    ", '" + GetModuleName() + "' " +
                    ") "
               );
         ...
         else
         {
            if (SQLGetData(2) == "")
            {
               debugVarObject("updating database for", OBJECT_SELF);
               SQLExecDirect(
                  "UPDATE nwn_creatures " +
                    " SET name = '" + GetName(OBJECT_SELF) + "' " +
                       ", cr = " + FloatToString(GetChallengeRating(
                             OBJECT_SELF)) +
                       ", race_id = " + IntToString(GetRacialType(
                             OBJECT_SELF)) + " " +
                       ", appearance_id = " + IntToString(GetAppearanceType(
                             OBJECT_SELF)) + " " +
                       ", gender_id = " + IntToString(GetGender(OBJECT_SELF)) +
                       ", is_pc = " + (GetIsPC(OBJECT_SELF) ? "TRUE" :
                             "FALSE") +
                       ", is_plot = " + (GetPlotFlag(OBJECT_SELF) ? "TRUE" :
                             "FALSE") +
                       ", is_immortal = " + (GetImmortal(OBJECT_SELF) ? "TRUE" :
                             "FALSE") +
                       ", is_lootable = " + (GetLootable(OBJECT_SELF) ? "TRUE" :
                             "FALSE") +
                       ", good_evil = " + IntToString(GetAlignmentGoodEvil(
                             OBJECT_SELF)) +
                       ", law_chaos = " + IntToString(GetAlignmentLawChaos(
                             OBJECT_SELF)) +
                       ", class1_id = " + IntToString(GetClassByPosition(1)) +
                       ", class1_level = " + IntToString(GetLevelByPosition(1)) +
                       ", class2_id = " + IntToString(GetClassByPosition(2)) +
                       ", class2_level = " + IntToString(GetLevelByPosition(2)) +
                       ", class3_id = " + IntToString(GetClassByPosition(3)) +
                       ", class3_level = " + IntToString(GetLevelByPosition(3)) +
                  " WHERE resref = '" + GetResRef(OBJECT_SELF) + "' " +
                    " AND ifnull(name, '') = '' "
                  );
            }
