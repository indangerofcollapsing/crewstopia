...
void main()
{
   ...
   if (GetSpawnInCondition(NW_FLAG_SPECIAL_COMBAT_CONVERSATION) &&
       GetIsPC(oPercep) && bSeen)
   {
      SpeakOneLinerConversation();
   }

   // March 5 2003 Brent
   // Had to add this section back in, since modifications were not taking
   // this specific example into account -- it made invisibility basically
   // useless.
   // If the last perception event was hearing based or if someone vanished
   ...
   else if (GetIsEnemy(oPercep) && bSeen)
   {
      ...
      if (!GetHasEffect(EFFECT_TYPE_SLEEP))
      {
         ...
         else
         {
            ...
            // @DUG DetermineCombatRound(); // Commented out for Fight-or-Flight
            ExecuteScript("fof_c2_default2", OBJECT_SELF); // @DUG
         }
      }
   }
   else
