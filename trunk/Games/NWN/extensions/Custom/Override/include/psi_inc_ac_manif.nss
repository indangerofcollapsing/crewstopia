                               ...
                               int nOptionFlags, int nResElemFlags, int nETchElemFlags, int nEBltElemFlags)
{
    ...
    // @DUG float fDur = 6.0 * GetManifesterLevel(manif.oManifester);
    // @DUG One round per level???  Are you fucking kidding me?
    float fDur = HoursToSeconds(GetManifesterLevel(manif.oManifester));
    if(manif.bExtend)
        fDur *= 2;
