...
int IsProficient(object oPC, int nBaseItem)
{
   ...
   switch(nBaseItem)
   {
       ...
       case BASE_ITEM_DWARVENWARAXE:
           if (GetHasFeat(FEAT_WEAPON_PROFICIENCY_EXOTIC, oPC) ||
               (GetHasFeat(FEAT_WEAPON_PROFICIENCY_MARTIAL, oPC) &&
                MyPRCGetRacialType(oPC) == RACIAL_TYPE_DWARF) || // @DUG
       ...
       case BASE_ITEM_ELF_LIGHTBLADE:
           if(GetHasFeat(FEAT_WEAPON_PROFICIENCY_EXOTIC, oPC)
              || MyPRCGetRacialType(oPC) == RACIAL_TYPE_ELF // @DUG
       ...
       case BASE_ITEM_CEP_TRIDENT:
           bProficient = GetHasFeat(FEAT_WEAPON_PROFICIENCY_MARTIAL, oPC);
           break;
       ...
       case BASE_ITEM_CEP_SAI:
           bProficient = GetHasFeat(FEAT_WEAPON_PROFICIENCY_EXOTIC, oPC) ||
              GetHasFeat(FEAT_WEAPON_PROFICIENCY_MONK, oPC);
           break;
       ...
       case BASE_ITEM_CEP_ASSASSINDAGGER:
           bProficient = GetHasFeat(FEAT_WEAPON_PROFICIENCY_SIMPLE, oPC) ||
              GetHasFeat(FEAT_WEAPON_PROFICIENCY_DRUID, oPC) ||
              GetHasFeat(FEAT_WEAPON_PROFICIENCY_MONK, oPC) ||
              GetHasFeat(FEAT_WEAPON_PROFICIENCY_ROGUE, oPC) ||
...
void DoWeaponEquip(object oPC, object oItem, int nHand)
{
    ...
    // Fix for eldritch glaive size
    if (GetTag(oItem) == "prc_eldrtch_glv") nWeaponSize = nRealSize + 1;
...
// @DUG    if(DEBUG) DoDebug("prc_restwpnsize - Weapon size: " + IntToString(GetWeaponSize(oItem)));
// @DUG    if(DEBUG) DoDebug("prc_restwpnsize - Character Size: " + IntToString(nSize));
...
// @DUG    if (nWeaponSize == nSize + 1 ||
    ...
    { // @DUG
       // Assign penalty
       debugVarObject("TWF penalty applied", oPC); // @DUG
