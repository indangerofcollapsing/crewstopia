...
// @DUG     int iJumpRoll = d20() + GetSkillRank(SKILL_JUMP, oPC) + iBonus + GetAbilityModifier(ABILITY_STRENGTH, oPC);
     object oEnemy = GetNearestCreature(CREATURE_TYPE_REPUTATION,
        REPUTATION_TYPE_ENEMY, oPC);
     int iJumpRoll = 0;
     if (oEnemy != OBJECT_INVALID && GetDistanceBetween(oPC, oEnemy) < 10.0f)
     {
        iJumpRoll += d20();
        FloatingTextStringOnCreature("Jump (" + IntToString(iJumpRoll) +
           ")...", oPC);
     }
     else
     {
        iJumpRoll = 20;
        FloatingTextStringOnCreature("Jump (take 20)...", oPC);
     }
     // skill 28 = jump
     ...
     iJumpRoll += GetSkillRank(SKILL_JUMP, oPC) + iBonus + GetAbilityModifier(ABILITY_STRENGTH, oPC); // @DUG
