...
void main()
{
   ...
   int nFloor;
   float fRaw = GetLocalFloat(OBJECT_SELF, VAR_COURAGE_MODIFIER) + 0.01;
   //debugVarFloat(GetName(OBJECT_SELF) + " courage", fRaw);
   float f_TheirCR;
   float f_OurCR;
   object oTheirs;
   object oMine;
   ...
   //debugVarInt(GetName(OBJECT_SELF) + " nFloor", nFloor);
   ...
   int nHPCheck;
   if (fMyPercentage < fEnemyPercentage ||
       GetLocalInt(OBJECT_SELF, "HEARD_THE_CALL") == 1)
   {
      nHPCheck = 1;
      //debugVarInt("nHPCheck", nHPCheck);
   }
   ...
   {
      //debug(GetName(OBJECT_SELF) + " checking morale");
      ...
      if (nDom == 0)
      {
         ...
         {
            // If a lair exists and is not close, run away to it
            if (getDistanceToLair(OBJECT_SELF) > 10.0f)
            {
               // Not in my lair, so run away
               //debug(GetName(OBJECT_SELF) + " retreating to lair");
               signalFlee(oEnemy, 0.0f); // original code: , fRange);
               ActionDoCommand(DetermineCombatRound());
            }
            else
            {
               // I'm in my lair, so either fight to the death, or surrender
               int nFriends = countNearbyFriends();
               //debugVarInt("friends nearby", nFriends);
               // A wise leader with many followers will try to save them by surrendering
               if (d20() + nFriends > GetAbilityScore(OBJECT_SELF, ABILITY_WISDOM))
               {
                  surrender();
               }
               else
               {
                  SpeakString("Fight to the death!", TALKVOLUME_TALK);
