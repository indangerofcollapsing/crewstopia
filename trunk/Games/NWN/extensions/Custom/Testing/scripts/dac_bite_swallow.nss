...
void main()
{
   ...
   {
      if (DEBUG) DoDebug("dac_bite_swallow: target is invalid");
      return;
   }
...
//   int nTargetSize = GetCreatureSize(oTarget);
   ...
   if (d4() > (nAttackerSize - nTargetSize))
   {
      FloatingTextStringOnCreature(GetName(oTarget) +
         " avoids being swallowed whole!", oTarget);
      return;
   }
   ...
   if (nHD == 0)
   {
      if (DEBUG) DoDebug("dac_bite_swallow: HD of self is zero: " + GetName(OBJECT_SELF));
   }
   ...
   FloatingTextStringOnCreature(GetName(oTarget) +
      ", due to a limitation in the script, has miraculously escaped!",
      oTarget);
}
