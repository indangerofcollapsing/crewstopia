// Does the item to be enhanced NOT already have the enhancement?
...
int StartingConditional()
{
   ...
   {
      logError("ERROR: invalid itemproperty in sc_enh_isneeded");
      return FALSE;
   }
   ...
   if (IPGetItemHasProperty(oItem, ip, nDurationType)) return FALSE;

   int nOldValue = GetGoldPieceValue(oItem);
   //debugVarInt("nOldValue", nOldValue);
   object oNewItem = CopyItem(oItem, OBJECT_SELF, TRUE);
   enhanceItem(oNewItem, ip, DURATION_TYPE_PERMANENT);
   if (! IPGetItemHasProperty(oNewItem, ip, DURATION_TYPE_PERMANENT))
   {
   ...
   if (nDurationType == DURATION_TYPE_TEMPORARY) nCostOfUpgrade /= 10;
