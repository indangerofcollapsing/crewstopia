// Could take level 1 in druid
int StartingConditional()
{
  if (GetLevelByClass(CLASS_TYPE_DRUID)>0) return FALSE;
  else if (GetAlignmentLawChaos(OBJECT_SELF)==ALIGNMENT_NEUTRAL) return TRUE;
  else if (GetAlignmentGoodEvil(OBJECT_SELF)==ALIGNMENT_NEUTRAL) return TRUE;
  else return FALSE;
}
