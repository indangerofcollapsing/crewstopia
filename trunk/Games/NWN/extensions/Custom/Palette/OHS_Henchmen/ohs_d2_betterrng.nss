...
{
  object oBetter = GetLocalObject(OBJECT_SELF,"OHS_BETTER_RANGED_WEAPON");
  object oNow = GetLocalObject(OBJECT_SELF,"OHS_FAVOURITE_RANGED_WEAPON");
  if (GetIsObjectValid(oBetter) && oBetter!=oNow)
  {
    SetCustomToken(793,GetName(oBetter));
    return TRUE;
  }
  else if (GetIsObjectValid(oNow))
  {
    SetCustomToken(793,GetName(oNow));
    return FALSE;
  }
  else
  {
    string sHandWeapon = OHS_GetStringByLanguage(201,GetPCSpeaker());
    SetCustomToken(793,sHandWeapon);
    return FALSE;
  }
}
